...
import ModalTambahAkun from "../../components/modal/ModalTambahAkun";
...
const MainContent = () => {
  ...
  const fetchGudangList = useCallback(async () => {
    try {
      const response = await axios.post(`${apiUrl}/akun/list`);
  ...
  const filteredItems = useMemo(() => {
    return gudangList.filter((gudang) =>
      gudang.username.toLowerCase().includes(filterValue.toLowerCase())
    );
  ...
  const handleDelete = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.post(`${apiUrl}/akun/delete`, { id });
          fetchGudangList();
