  ...
  const submitData = async () => {
    ...
    if (aksi === "update") {
      ...
      Swal.fire({
        ...
        html: "Apakah kamu yakin ingin mengubah <b>Pengeluaran</b> ini?",
        icon: "warning",
          ...
          try {
            if (data && data.data && data.data.data) {
              ...
              if (res.data.status === true) {
                Swal.fire({
                  title: "Success",
                  html: "<b>Pengeluaran</b> berhasil diubah",
                  icon: "success",
                }).then((result) => {
                  if (result.isConfirmed) {
      ...
      Swal.fire({
        title: "Apakah Kamu Yakin ?",
        html: "Apakah kamu yakin ingin Menambahkan <b>Pengeluaran</b> ini?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
          try {
            if (data && data.data && data.data.data) {
              const res = await axios.post(`${apiUrl}/pengeluaran/posting`, data.data.data);
              if (res.data.status === true) {
                Swal.fire({
                  title: "Success",
                  html: "<b>Pengeluaran</b> berhasil ditambah",
                  icon: "success",
                }).then((result) => {
                  if (result.isConfirmed) {
                    router.push("/pengeluaran");
          ...
          <TableBody>
            {item?.map((item: { name: string; kode: string; quantity: number; price: string; gudang: string; amount: string; variable: string; discount: string; lots: string }, index: number) => (
              <TableRow key={index} className="">
                ...
                <TableCell className="text-left">{item.discount || 0}%</TableCell>
