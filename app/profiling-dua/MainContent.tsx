...
const MainContent = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      let response;
      if (gudang && gudang !== "0") {
        response = await axios.post(`${apiUrl}/profile/ListByCustomer`, {
          nama: gudang,
        });
  ...
  const handleInputChange2 = useCallback(
    (index: number, key: keyof User, value: string, id: number) => {
  ...
  const renderCell = useCallback(
        ...
        return (
          <div className="flex gap-2">
            <Button
              ...
              size="sm"
              onClick={() =>
                router.push(`/profiling-dua/form-edit?id=${user.id}`)
              }
            >
              Edit
            </Button>
    ...
    [handleInputChange2, router], // tambahkan router di dalam dependency array
  );
